    ...
    <div className="min-h-screen bg-[#F9FAFB] flex flex-col">
      ...
      <header className="shrink-0 bg-white border-b border-gray-100 px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-base font-semibold text-gray-800">AI Mentor</h1>
            <div className="flex items-center gap-1">
              <WifiOff size={10} className="text-emerald-500" />
              <span className="text-[10px] text-emerald-600 font-medium">Offline Ready</span>
            </div>
          </div>
        </div>
      </header>
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            >
              ...
              <p className={`text-[10px] mt-1 ${msg.sender === 'user' ? 'text-violet-200' : 'text-gray-400'}`}>
                {msg.timestamp}
              </p>
            </div>
          </div>
      ...
      {messages.length <= 2 && (
        <div className="shrink-0 px-4 pb-2">
          <p className="text-[10px] text-gray-400 mb-2">Quick Questions:</p>
          <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-2">
            {suggestedPrompts.map((prompt) => (
              <button
                key={prompt}
                onClick={() => sendMessage(prompt)}
                className="px-3 py-2 bg-white border border-orange-200 rounded-xl text-xs text-gray-700 whitespace-nowrap active:scale-95 transition-transform select-none hover:bg-orange-50"
              >
      ...
      <div className="shrink-0 p-4 bg-white border-t border-gray-100">
        <div className="flex items-center gap-2">
          <button className="w-10 h-10 bg-orange-100 rounded-full flex items-center justify-center active:scale-95 transition-transform shrink-0">
